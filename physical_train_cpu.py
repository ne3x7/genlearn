...
def main(args):
    ...
    if args.sparse:
        data_tx, data_vx, data_ty, data_vy = items_sparse[:len_train, :, :2],\
                                             items_sparse[len_train:, :, :2],\
                                             items_sparse[:len_train, :, 2:],\
                                             items_sparse[len_train:, :, 2:]
    else:
    ...
    bunch = PhysGenLearnDataBunch.create(train_ds=x, valid_ds=y, bs=bs, path='ode')
    ...
    rhs = RHS(c).double()
    if args.sparse:
        ode = ODE2(rhs, 2, t, torch.from_numpy(initial_x)).double()
    else:
        ode = ODE(rhs, 2, t, torch.from_numpy(initial_x)).double()
